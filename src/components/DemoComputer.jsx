...
const DemoComputer = (props)  => {
    ...
    // Define a scale factor to make the mobile video appear larger
    const scaleFactor = isMobileVideo ? 1.15 : 1; // 1.5 means 50% larger for mobile video
        ...
        <group ref={group} {...props} dispose={null}>
            <group name="Scene">
                ...
                >
                    ...
                    { isMobileVideo && (
                        // For the mobile video, use a plain base material (or any fallback)
                        <meshBasicMaterial color="black" />
                    )}
                </mesh>
                { isMobileVideo && (
                    // Overlay a plane with the mobile video's original aspect ratio
                    <mesh
                        // Position the plane exactly over the monitor screen (tweak if needed)
                        position={[0.127, 1.831, 0.55]}
                        rotation={[0, 0, 3.15]}
                        scale={[-scaleFactor, scaleFactor, scaleFactor]}
                    >
                        {/*
              Use a plane geometry:
              - The base height is 0.608 (from your monitor dimensions)
              - The width is calculated based on the video frame ratio (490/986 ≈ 0.497)
              - Multiplying both by scaleFactor to make the video appear bigger
            */}
                        <planeGeometry args={[0.608 * (490 / 986) * scaleFactor, 0.608 * scaleFactor]} />
                        <meshBasicMaterial map={txt} />
                    </mesh>
                )}
                <group
